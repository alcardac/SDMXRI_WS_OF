...
namespace Estat.Nsi.DataRetriever.Builders
{
    ...
    internal sealed class SeriesDataSetSqlBuilder : SqlBuilderBase, ISqlBuilder
    {
        ...
        private static void GenerateDataSetSql(DataRetrievalInfoSeries info)
        {
            ...
            if (!info.DataSetAttributes.Any(mapping => mapping.Mapping.Columns.Count > 0))
            {
                return;
            }
            ...
            try
            {
                ...
                string sql = GenerateSelect(true, ConvertToMapping(info.DataSetAttributes));
            ...
            // log for easy debug
            Logger.Info(string.Format(CultureInfo.InvariantCulture, Resources.InfoGeneratedSQLFormat1, info.DataSetSqlString));
