...
namespace Xml.Schema.Linq {
    ...
    public partial class XTypedElement : IXMetaData, IXTyped, IXmlSerializable {
        ...
        XName IXMetaData.SchemaName {
            get {
                return XName.Get("anyType", XmlSchema.Namespace);
            }
        }
        ...
        XTypedElement IXMetaData.Content { //Content is non-null only for root element wrapper types
            get {
                return null; 
            }
        }
        ...
        private void CheckXsiNil(XElement parentElement) {
            XAttribute xsiNil = parentElement.Attributes(XName.Get("nil", XmlSchema.InstanceNamespace)).FirstOrDefault();
            if (xsiNil != null && xsiNil.Value == "true") { //Since we are adding content
                xsiNil.Remove();
            }
        }
    ...
    public static class XTypedServices {
        ...
        public static readonly Dictionary<System.Type, System.Type> EmptyTypeMappingDictionary = new Dictionary<System.Type, System.Type>();
        ...
        public static W ToXTypedElement<W, T>(XElement xe, ILinqToXsdTypeManager typeManager) where T : XTypedElement where W : XTypedElement {
        ...
        public static XTypedElement ToXTypedElement(XElement xe, ILinqToXsdTypeManager typeManager, Type rootType, Type contentType) {
            ...
            if (rootElement == null) { //If not already created
                XName instanceElementName = xe.Name; //Storing the name since SetName() called from the functional const of rootType will update the name to that of the root type
                ...
                if (constInfo != null) {
                    ...
                    throw new LinqToXsdException(contentType.ToString() + " is not an expected content type for root element type " + rootType.ToString());
                }                
        ...
        public static T ToXTypedElement<T>(XElement xe) where T : XTypedElement, new() {
            ...
            if (xoSubType == null) { //No association bet XTypedElement and xelement
                xoSubType = new T();
        ...
        public static XTypedElement ToXTypedElement(XElement xe, ILinqToXsdTypeManager typeManager, System.Type t) {
            ...
            XTypedElement xoSubType = GetAnnotation(t, xe); //Try getting back as the type first, optimized for the cases where xsi:type cannot appear
            if (xoSubType == null) { //Try xsi:type and lookup in the typeDictionary
                ...
                if (clrType != null) {
                    ...
                    if(!t.IsAssignableFrom(clrType)) { //xsi:type is not subtype of schema type
                        clrType = t;
                    }
                }    
                else { //xsi:type not present or CLRType not found for xsi:type name 
                    clrType = t;
                }                
        ...
        public static XTypedElement ToXTypedElement(XElement xe, ILinqToXsdTypeManager typeManager) {
            ...
            Type xsiClrType = GetXsiClrType(xe, typeManager); //Check for global xsi:type, tag might be a fragment or unknown element name
            if (xsiClrType != null) {
        ...
        public static T CloneXTypedElement<T>(T xTypedElement) where T : XTypedElement, new() {
            ...
            clonedElement.AddAnnotation(new XTypedElementAnnotation(newObject)); //Need to set up association for the cloned type
            return newObject;
        }
        ...
        public static T Load<T>(XmlReader reader) where T : XTypedElement, new()
        {
            XDocument doc = XDocument.Load(reader);
            XElement xeroot = doc.Root;
            return ToXTypedElement<T>(xeroot);
        ...
        /// <summary>
        /// Load an XML document from a <paramref name="reader"/> and return it as <typeparamref name="W"/>
        /// </summary>
        /// <typeparam name="W">The element type</typeparam>
        /// <typeparam name="T">The element </typeparam>
        /// <param name="reader">The input document</param>
        /// <param name="typeManager">The Type manager</param>
        /// <returns>The <paramref name="reader"/> as <typeparamref name="W"/></returns>
        /// <remarks>ADDED by TCH</remarks>
        public static W Load<W, T>(XmlReader reader, ILinqToXsdTypeManager typeManager)
            where T : XTypedElement
            where W : XTypedElement
        {
            XDocument doc = XDocument.Load(reader);
        ...
        public static W Parse<W, T>(string xml, ILinqToXsdTypeManager typeManager) where T : XTypedElement where W : XTypedElement {
        ...
        public static void SetContentToNil(XTypedElement xTypedElement) {
            ...
            if (thisElement.Attribute(xsiNil) == null) { //xsi:nil attribute not already present
        ...
        public static void SetName(XTypedElement root, XTypedElement type) {
            ...
            currentElement.AddAnnotation(new XTypedElementWrapperAnnotation(root));  //The XElement tree is hooked up to both root type as well as content type?
        ...
        public static XTypedElement GetCloneIfRooted(XTypedElement innerType) {
            ...
            if (wrapperAnnotation != null) { //Already rooted to a wrapper
        ...
        public static void SetList<T> (IList<T> list, IList<T> value) {
            if ((object)list == (object)value) { //set property to itself, do nothing
                return;
            }
            list.Clear();
        ...
        public static IList<T> ParseListValue<T>(XAttribute attribute, XmlSchemaDatatype datatype) {
            ...
            return ParseListValue<T>(attribute.Value, attribute.Parent, attribute.Name, ContainerType.Attribute, datatype);
        }
        ...
        private static object ParseUnionValue(string value, XElement element, XName itemXName, SimpleTypeValidator typeDef, ContainerType containerType) {
            ...
            object typedValue;
            Exception e = unionDef.TryParseValue(value, NameTable, new XNamespaceResolver(element), out matchingType, out typedValue);
            
            ListSimpleTypeValidator listType = matchingType as ListSimpleTypeValidator;
            if (listType != null) {
                SimpleTypeValidator itemType = listType.ItemType;
                return new XListContent<object>((IList)typedValue, element, itemXName, containerType, itemType.DataType);
            }
            Debug.Assert(e == null);
        ...
        internal static XTypedElement GetAnnotation(Type t, XElement xe) {
            ...
            if (xoWrapperAnnotation != null) { //Return the root type if the element is annotated with the root
                xObj = xoWrapperAnnotation.typedElement;
                if (t.IsAssignableFrom(xObj.GetType())) { //Check if we are asking for element wrapper
                    return xObj;
                }
            }
            ...
            if (xoAnnotation != null) {
                ...
                if (t.IsAssignableFrom(xObj.GetType())) { //Check if we are asking for type
                    return xObj;
                }
            }
            return null;
        ...
        internal static T ParseValue<T>(string value, XElement element, XmlSchemaDatatype datatype) {
            ...
            // HACK tasos added for 2.1 Generic.BaseValueType.value auto-generated code
            if (datatype.TypeCode == XmlTypeCode.AnyAtomicType && typeof(T) == typeOfString)
            {
                return (T)XmlSchemaType.GetBuiltInSimpleType(XmlTypeCode.String).Datatype.ChangeType(value, typeOfString);
            }
        ...
        internal static string GetXmlString(object value, XmlSchemaDatatype datatype, XElement element) {
            ...
            {
                stringValue = XmlConvert.ToString((DateTime)value, XmlDateTimeSerializationMode.RoundtripKind);
            }
            else {
                ...
                if (datatype.TypeCode != XmlTypeCode.AnyAtomicType || stringValue == null)
                {
        ...
        internal static XElement GetXElement(XTypedElement xObj, XName name) {
            ...
            if (newElement.Parent != null) { //Element/XTypedElement already added to the tree, need to clone XTypedElement
                newElement = xObj.Clone().Untyped;
            }
            IXMetaData metaData = xObj as IXMetaData;
            Debug.Assert(metaData != null);
            if (metaData.TypeOrigin == SchemaOrigin.Fragment) { //Set correct element name as the name of the property/element
        ...
        private static string ParseQName(string qName, out string prefix) {
            ...
            if (colonPos == -1) { //the whole name is the localName and prefix is empty
                return qName;
            }
            else {
        ...
        private static bool TypeValid(XTypedElement typedElement, XName instanceName) {
            ...
            if(metaData.SchemaName.Equals(instanceName)) { //Element names match
                return true;
            }
            return false;
        }
    ...
    public abstract class XListVisualizable
    {
        internal class XListDebugVisualizer
        {
            ...
            public XTypedElement[] Items {get{XTypedElement[] tArray = new XTypedElement[((ICountAndCopy)_xList).Count]; ((ICountAndCopy)_xList).CopyTo(tArray, 0); return tArray;}}
        }
    }
    ...
    public abstract class XList<T> : XListVisualizable, IList<T>, ICountAndCopy {
        ...
        public bool Remove(T value) {
            ...
            if (x != null) { //Found it in the list
                element.Remove();
                return true;
            }
            return false;
        ...
        protected IEnumerator<XElement> GetListElementsEnumerator() {
            if (container.ValidationStates == null) {
                ...
                else { //Need to enumerate through all members of the subst group
     ...
     public class XTypedList<T> : XList<T> where T : XTypedElement {
        ...
        public XTypedList(XTypedElement container, ILinqToXsdTypeManager typeManager, XName itemXName) : base(container, itemXName) {
    ...
    public class XSimpleList<T> : XList<T> {
        ...
        public XSimpleList(XTypedElement container, XmlSchemaDatatype dataType, XName itemXName) : base(container, itemXName) {
    ...
    internal class XTypedElementWrapperAnnotation {  //Seperate class for annotating root elements
        internal XTypedElement typedElement;
