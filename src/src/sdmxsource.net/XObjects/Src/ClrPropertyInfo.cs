...
namespace Xml.Schema.Linq.CodeGen {
    internal class XCodeTypeReference : CodeTypeReference
    {
        ...
        public XCodeTypeReference(string typeName, params CodeTypeReference[] typeArguments) : base(typeName, typeArguments)
        {
        }
    ...
    internal partial class ClrWildCardPropertyInfo : ClrBasePropertyInfo {
        ...
        internal override XCodeTypeReference ReturnType {
            get {
               return new XCodeTypeReference("IEnumerable", new CodeTypeReference(Constants.XElement));
            }
    ...
    internal class ClrWrappingPropertyInfo : ClrBasePropertyInfo {
        ...
        private string CheckPropertyName(string className) {
            if (this.propertyName.Equals(className)) { //This can happen as property names are checked against the wrapped complex type name and not against the global element name
    ...
    internal partial class ClrPropertyInfo : ClrBasePropertyInfo 
    {
        ...
        internal ArrayList SubstitutionMembers {
            ...
            set {
                substitutionMembers = value;
            }
        }
        
        internal bool IsSubstitutionHead {
            get {
                return substitutionMembers != null;
            }
        }
        ...
        internal override bool IsList {//This is for repeating elements, not schema list
            get {
                return (propertyFlags & PropertyFlags.IsList) != 0;
            }
            set {
                if (value) {
        ...
        internal override bool IsNullable {
            get {
                return (propertyFlags & PropertyFlags.IsNullable) != 0 && fixedDefaultValue == null;
            }
            set {
                if (value) {
        ...
        internal bool Validation {
            get {
                return this.typeRef.Validate && !IsRef;
            }
        }

        internal override bool FromBaseType {
            get {
                return (propertyFlags & PropertyFlags.FromBaseType) != 0;
            }
            set {
                if (value) {
        ...
        internal override bool IsDuplicate {
            get {
                return (propertyFlags & PropertyFlags.IsDuplicate) != 0;
            }
            set {
                if (value) {
            ...
            getStatements.Add(
                new CodeVariableDeclarationStatement(
                        ...
                        CodeDomHelper.CreateMethodCall(
                                ...
                                substParams)));
            CheckOccurrence(getStatements);
            getStatements.Add(new CodeMethodReturnStatement(new CodeCastExpression(ReturnType, new CodeVariableReferenceExpression("x"))));                                
        }
        ...
        private CodeExpression[] GetListParameters(bool set, bool constructor) {
            ...
            if (set) { //Value or propertyName in const
                paramCount++;
                if (constructor) {
            ...
            if (this.IsSubstitutionHead) {
                foreach(XmlSchemaElement elem in substitutionMembers) {
                    listParameters[paramIndex++] = CodeDomHelper.XNameGetExpression(elem.QualifiedName.Name, elem.QualifiedName.Namespace);
                }
            }
            else {
        ...
        protected void CreateFixedDefaultValue(CodeTypeDeclaration typeDecl)
        {
            if (fixedDefaultValue != null)
            {
                ...
                if (FixedValue != null)
                {
                    fixedOrDefaultField = new CodeMemberField(returnType, NameGenerator.ChangeClrName(PropertyName, NameOptions.MakeFixedValueField));
                }
                else// if (DefaultValue != null)
                {
                    fixedOrDefaultField = new CodeMemberField(returnType, NameGenerator.ChangeClrName(PropertyName, NameOptions.MakeDefaultValueField));
                }

                CodeDomHelper.AddBrowseNever(fixedOrDefaultField);
                fixedOrDefaultField.Attributes = (fixedOrDefaultField.Attributes & ~MemberAttributes.AccessMask & ~MemberAttributes.ScopeMask)
                                                | MemberAttributes.Private | MemberAttributes.Static;

                fixedOrDefaultField.InitExpression = SimpleTypeCodeDomHelper.CreateFixedDefaultValueExpression(returnType, fixedDefaultValue);
            ...
            if (this.FixedValue != null) {
                ctor.Statements.Add(
                      new CodeAssignStatement(
                            ...
                            CodeDomHelper.CreateFieldReference(null, NameGenerator.ChangeClrName(propertyName, NameOptions.MakeFixedValueField))));
            }
            else if (DefaultValue != null) {
                  ctor.Statements.Add(
                      new CodeAssignStatement(
                            CodeDomHelper.CreateFieldReference(null, propertyName),
                            CodeDomHelper.CreateFieldReference(null, NameGenerator.ChangeClrName(propertyName, NameOptions.MakeDefaultValueField))));
            }
        }
