...
namespace Org.Sdmxsource.Sdmx.Structureparser.Builder.BaseObjects.Impl {
	...
	public class AttributeSuperBeanBuilder : 
			ComponentSuperBeanBuilder<IAttributeObjectBase, IAttribute> {
	
		public override IAttributeObjectBase Build(IAttribute buildFrom,
				ISdmxRetrievalManager retrievalManager, SuperBeans existingBeans) {
			ICodelistObjectBase codelistBean = base.GetCodelist(buildFrom,retrievalManager,existingBeans);
			IConceptObjectBase conceptSuperBean = base.GetConcept(buildFrom,retrievalManager,existingBeans);
			return new AttributeObjectBaseCore(buildFrom, codelistBean,
					conceptSuperBean);
		}
	}
}
