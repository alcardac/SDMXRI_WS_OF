...
namespace Org.Sdmxsource.Sdmx.Structureparser.Builder.BaseObjects.Impl {
	...
	public class ConceptSchemeSuperBeanBuilder : 
			StructureBuilderImpl<IConceptSchemeObjectBase, IConceptSchemeObject> {
	
		private CodelistSuperBeanBuilder codelistSuperBeanBuilder;
	
		public override IConceptSchemeObjectBase Build(IConceptSchemeObject buildFrom,
			...
			IDictionary<IConceptObject, ICodelistObjectBase> codelistMap = new Hashtable<IConceptObject, ICodelistObjectBase>();
			/* foreach */
			foreach (IConceptObject conceptBean  in  buildFrom.Items) {
				if (conceptBean.CoreRepresentation != null
						&& conceptBean.CoreRepresentation
								.RepresentationRef != null) {
					IMaintainableRefObject codelistRef = conceptBean
					...
					if (codelistSuperBean == null) {
						...
						if (codelistSuperBean == null) {
							throw new ReferenceException(conceptBean
									.CoreRepresentation.RepresentationRef);
						}
					}
	
					ILOG.J2CsMapping.Collections.Collections.Put(codelistMap,conceptBean,codelistSuperBean);
				}
			}
			return new ConceptSchemeObjectBaseCore(buildFrom, codelistMap);
		}
