...
namespace Org.Sdmxsource.Sdmx.Structureparser.Builder.BaseObjects.Impl {
	...
	public class DimensionSuperBeanBuilder : 
			ComponentSuperBeanBuilder<IDimensionObjectBase, IDimension> {
	
		public override IDimensionObjectBase Build(IDimension buildFrom,
			...
			try {
				...
				if (buildFrom.IsMeasureDimension()) {
					//IMPORTANT - THIS NEEDS TO GET THE CONCEPT SCHEME IF IT HAS ONE
					return new DimensionObjectBaseCore(buildFrom, null, concept);
				} else {
					ICodelistObjectBase codelist = GetCodelist(buildFrom,
							retrievalManager, existingBeans);
					return new DimensionObjectBaseCore(buildFrom, codelist, concept);
				}
			} catch (Exception th) {
