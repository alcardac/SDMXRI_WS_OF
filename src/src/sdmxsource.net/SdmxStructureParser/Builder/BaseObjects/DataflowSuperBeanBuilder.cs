...
namespace Org.Sdmxsource.Sdmx.Structureparser.Builder.BaseObjects.Impl {
	...
	public class DataflowSuperBeanBuilder : 
			StructureBuilderImpl<IDataflowObjectBase, IDataflowObject> {
		private static readonly log4net.ILog LOG = log4net.LogManager
				.GetLogger(typeof(SuperBeansBuilderImpl));
	
		private DataStructureSuperBeanBuilder dataStructureSuperBeanBuilder;
	
		public override IDataflowObjectBase Build(IDataflowObject buildFrom,
			...
			IMaintainableRefObject dsdRef = buildFrom.DataStructureRef
					.MaintainableReference;
	
			LOG.Debug("Build Dataflow ISuper. Get DSD Superbean :" + dsdRef);
	
			IDataStructureObjectBase kfSuperBean = superBeanRetrievalManager
					.IGetDataStructureObjectBase(dsdRef);
	
			if (kfSuperBean == null) {
				LOG.Debug("No existing dsd super bean found, build new");
				IDataStructureObject keyFamily = retrievalManager
						.GetDataStructure(dsdRef);
				if (keyFamily != null) {
					kfSuperBean = dataStructureSuperBeanBuilder.Build(keyFamily,
							retrievalManager, existingBeans);
					existingBeans.AddDataStructure(kfSuperBean);
				} else {
					throw new ReferenceException(
							"Can not resolve reference to data structure: "
									+ dsdRef);
				}
			}
			return new DataflowObjectBaseCore(buildFrom, kfSuperBean);
		}
	}
}
